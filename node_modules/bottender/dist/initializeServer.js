...
function initializeServer({ isConsole, config, } = {}) {
    ...
    const { initialState, plugins, session, channels } = merge_1.default(bottenderConfig, config);
    ...
    const sessionDriverConfig = ((session && session.stores) || {})[sessionDriver] || {};
    const sessionStore = new SessionStore(sessionDriverConfig, session && session.expiresIn);
