...
class Bot {
    ...
    createRequestHandler() {
        ...
        return (inputBody, requestContext) => __awaiter(this, void 0, void 0, function* () {
            ...
            const sessionKey = this._connector.getUniqueSessionKey(body, requestContext);
            let sessionId;
            let session;
            if (sessionKey) {
                ...
                yield this._connector.updateSession(session, body);
            }
            const events = this._connector.mapRequestToEvents(body);
            const contexts = yield p_map_1.default(events, event => this._connector.createContext({
                event,
                session,
            ...
            if (this._sync) {
                try {
                    ...
                    if (sessionId && session) {
                        ...
                        yield this._sessions.write(sessionId, session);
                    }
                }
                catch (err) {
                    console.error(err);
